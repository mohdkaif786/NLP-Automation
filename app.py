...
# creatw space between two context
def space():
...
st.markdown("<strong> <p style='color: #424874'>3) Different types of metrics formation (Count Vectorizing, TF-IDF)</p></strong>", unsafe_allow_html= True)
...
def hyperparameter():
    try:
        ...
        max_featues = st.selectbox("Maximum Features you want to restrict?", features)
        space()
        space()
        ranges = ["1,1", "1,2","1,3"]
        ngram_range = st.selectbox("Combination of words", ranges).split(',')
        return max_featues, ngram_range
...
def app():
    ...
    if df is not None:
        ...
        space()
        target = st.selectbox("Select Targeet Column", data.columns)
        space()

        #Reassinging feature to DataFrame
        ...
        # Calling function for Preprocessing , Bag of Words , Target variable
        ...
        def sentimental(text):
            ...
            def getSubjectivity(text):
                return TextBlod(text).sentiment.subjectivity
            
            # Create a function to get the polarity 
            def getPolarity(text):
                return TextBlod(text).sentiment.polarity
        ...
        if st.button("submit"):
            ...
            if preprocessor == "Stemming":
                ...
                X_train, X_test, y_tain, y_test = nlp_model.spilt_data(X,y)
